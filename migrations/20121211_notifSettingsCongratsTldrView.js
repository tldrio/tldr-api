...
, function (cb) {
    ...
    console.log('Adding congratsTldrViews options to notificationsSettings ');
    ...
    User.find({}, function(err, users) {
      ...
      , function (cb) {
          ...
          var prevSettings = users[i].notificationsSettings
            , newSettings = _.extend({}, prevSettings, { congratsTldrViews: true });
          users[i].notificationsSettings = newSettings;
          console.log('Adding congratsTldrViews to: ' + users[i]._id);
...
, function (cb) {
    User.find({ "notificationsSettings.congratsTldrViews": { $exists: false } }, function(err, users) {
      if (users.length === 0) {
