...
describe('Topic', function () {
  ...
  it('createAndSaveInstance creates slugs correctly', function (done) {
    var topicData1 = { type: 'category', name: 'Same as AlWays' }
      , topicData2 = { type: 'domain', name: 'domain.com' }
      ;

    Topic.createAndSaveInstance(topicData1, function (err) {
      Topic.createAndSaveInstance(topicData2, function (err) {
        Topic.find({}, function (err, topics) {
          topics[0].slug.should.equal('same-as-always');
          topics[1].slug.should.equal('domain.com');
          done();
        });
      });
    });
