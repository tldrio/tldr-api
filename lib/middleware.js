...
/**
 * Many website routes need common values like the logged user
...
/**
 * Set the languages for which to display tldrs
 * The querystring is used if it set, else it is the cookie
 * The cookie is used to remember the settings from session to session
 * The querystring doesn't change the cookie settings
 * Defaults to ['en'] (only English) if nothing selected
 */
function populateLanguages (req, res, next) {
  var languages;

  if (req.query.languages) {
    languages = req.query.languages.split(',');
  } else {
    languages = req.cookies.languages || ['en'];
    ...
    if (! languages.constructor || languages.constructor.name !== 'Array') { languages = ['en']; }
    if (languages.length === 0) { languages = ['en']; }

    // Refresh cookie or set it if it is the first time
    res.cookie('languages', JSON.stringify(languages), { path: '/', maxAge: 365 * 24 * 3600 });
  }

  req.renderingValues = req.renderingValues || {};
  req.renderingValues.languages = languages;
...
module.exports.populateLanguages = populateLanguages;
