...
function calculateRenderingValues (req, cb) {
  ...
  values.pageMetaProperties['og:title'] = 'tl;dr';
  values.pageMetaProperties['og:type'] = 'website';
  values.pageMetaProperties['og:url'] = 'http://tldr.io';
  values.pageMetaProperties['og:image'] = 'http://tldr.io/assets/img/fbicon.png';
  values.pageMetaProperties['og:site_name'] = 'tldr.io';
  values.pageMetaProperties['og:description'] = 'Interesting Content, Summarized by People';
  values.pageMetaProperties['fb:app_id'] = '250458838406360';

  // Lambda used to iterate over the meta properties
  values.showPageMetaProperties = function (text) {
    var mp = this.pageMetaProperties, prop, res = "";

    for (prop in mp) {
      if (mp.hasOwnProperty(prop)) {
        res += h4e.render(text, {values: { property: prop, content: mp[prop] }});
      }
    }
