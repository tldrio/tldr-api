...
function CORS (req, res, next) {
  ...
  var protocol = (req.protocol && req.protocol === 'https') ? "https://" : "http://"
    ...
    , originToAllow = protocol + config.origin   // Base case: we only allow requests from tldr.io
    ;
...
function logAPIUsage (req, res, next) {
  ...
  if (!name || !key) { return next(); }   // This is not a public URL call
  ...
  APIClient.findOne({ name: name }, function (err, apic) {
    ...
    apic.incrementRouteUsage(req.path, function (err) {
      return next();
    });
  });
}
