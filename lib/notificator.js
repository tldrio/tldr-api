...
// Detect language of tldr with Google Translate API
function detectTldrLanguage (data) {
  var tldr = data.tldr
    , longestBullet= _.max(tldr.summaryBullets, function (bullet) { return bullet.length; })
    ;

  if (config.env === 'test') { return; }   // We're not here to test their API

  request.get({ headers: {'Accept': 'application/json'}
              , uri: 'https://www.googleapis.com/language/translate/v2/detect?key=AIzaSyCUijyDhqn64dHllt6JP15cOqyEZFeH2NI&q=' + longestBullet
              },
              function (error, response, body) {
                var infos, language, confidence;
                try {
                  infos = JSON.parse(response.body);
                  language = infos.data.detections[0][0].language;
                  confidence = infos.data.detections[0][0].confidence;
                } catch (e) {
                  // No need to do anything but API shouldnt crash because of a problem in the JSON
                  bunyan.error(e);
                }
                Tldr.update({ _id: tldr._id }, { $set: { language: { language: language , confidence: confidence } } }, { multi: false }).exec();
              });
}


// Register event handlers
mqClient.on('tldr.created', detectTldrLanguage);

mqClient.on('tldr.read', updateTldrReadCount);
mqClient.on('tldr.read', function () { app.incrementTotalTldrReadCount(); });
mqClient.on('tldr.read', function (data) { app.incrementTotalWordsSaved(data.tldr.articleWordCount || 275); });
