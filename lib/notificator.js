...
/**
 * If a tldr is on the whitelist, the creator gets 7 days of free
 */
function updateCreatorIfTldrOnWhitelist (data) {
  var tldr = data.tldr
    , creator = data.creator
    ;

  if (!tldr || !creator) { return; }

  whitelist.isWhitelisted(tldr.originalUrl, function (err, iswl) {
    if (!err && iswl) {
      User.update({ _id: creator._id }, { $set: { lastWhitelistedTldr: tldr._id, lastWhitelistedTldrAt: new Date() } }, { multi: false }, function () {});
    }
  });
}
...
/**
 * Whitelist urls given in a data message
 */
function whitelistUrls (data) {
  if (!data.urls) { return; }
  whitelist.whitelist(data.urls);
}


// Triggers
mqClient.on('tldr.created', updateCreatorIfTldrOnWhitelist);
...
mqClient.on('searchBatch.hackernews', whitelistUrls);
