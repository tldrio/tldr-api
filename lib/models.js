...
function setTldrCreator(theTldr, creator, callback) {
  ...
  theTldr.contributors.push(creator);
  theTldr.save(function(err, tldr) {
    if (err) {throw err;}

    if (creator.tldrsCreated.length === 0) {
      // Create link to page and send congratulory email
      link = config.apiUrl + '/tldrs/search?url=' + encodeURIComponent(tldr.url) ;
      mailer.sendEmail({ type: 'congratulationsFirstTldr'
                       , to: creator.email
                       , values: { link: link } }
        , function(error, response) { if(error){ bunyan.warn('Error sending congratulory first tldr email', error); } });
    }
