 ...
 * @param {Number} daysAgo TimeLapse in days for which to compute the analytics. Computes analytics for allTime if null
 * @param {Function} callback callback once the computation is done. Signature function (err, analytics, rawData)
 */
function getAnalyticsForUser (user, daysAgo, callback) {
  ...
  values = { selection: selectionName };
