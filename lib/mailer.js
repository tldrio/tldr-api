...
function sendEmail (options, cb) {
  var type = options.type
    ...
    , from = options.from || 'tl;dr <hello@tldr.io>'
  ...
  if (sendEnvironments[config.env]) {
    hogan(process.env.TLDR_API_DIR + '/templates/emails/' + type + '.mustache', values, function(err, result) {
      var mailOptions = { from: from
                        , to: to
                        , subject: environmentPrefixes[config.env] + i18n[type]
                        , html: result
                        };
