...
function sendConfirmToken (type, to, values, callback) {
  var link;

  if (config.env === 'production' || config.env === 'development') {
    link = config.websiteUrl +
           '/confirm?confirmEmailToken=' +
           encodeURIComponent(values.user.confirmEmailToken) +
           '&email=' +
           encodeURIComponent(values.user.email);

    hogan('./templates/emails/' + type + '.mustache', { username: values.user.username, link:link, cache: true }, function(err, result) {
