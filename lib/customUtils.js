...
function normalizeProtocolAndHostname (hostname, protocol, port) {
  var res;
...
function normalizeQuerystring (hostname, query, search) {
  ...
  if (hostnamesUsingQueryStrings[hostname]) {
...
function normalizeUrl (theUrl) {
  ...
  nUrl += normalizeProtocolAndHostname(hostname, parsedUrl.protocol, parsedUrl.port);
  nUrl += normalizePath(parsedUrl.pathname);
