...
function createDataForUnsubscribeLink (text, expiration) {
  var hmac = crypto.createHmac('sha1', config.unsubscribeKey )
    , expiration = expiration || new Date().setDate(new Date().getDate() + config.unsubscribeExpDays); // 48h expiration by default

  hmac.update(text + '/' + expiration);

  return { signature: hmac.digest('hex'), expiration: expiration };
}
