...
function slugify (input) {
  ...
  // And use only dashes as delimiters
...
/**
 * Create an unused slug for this model
 * @param {Object} instance The model
 * @param {String} field The field to slugify
 * @param {String} slugField The field that will contain the slug.
 *                           The field must of course exist and it is good to flag it as unique
 * @param {Function} callback Signature: err, instance
 *
 */
function createUnusedSlug (instance, field, slugField, callback) {
  var Model = instance.constructor
    , slugBody = slugify(instance[field]).substring(0, config.slugSize)
    , query = {}
    , max = 0
    ;

  query[slugField] = new RegExp('^' + slugBody + '$');
  Model.findOne(query, function (err, doc) {
    if (err) { return callback(err); }

    if (!doc) {   // Simple case where the slug was never seen before
      instance[slugField] = slugBody;
      return callback(null, instance);
    } else {       // Slugs from the same family were found, craft a not yet used one
      query[slugField] = new RegExp('^' + slugBody + '-[0-9]*$');
      Model.find(query, function (err, docs) {
        _.each(docs, function (_doc) {
          var numberPart = _doc[slugField].substring(slugBody.length + 1);
          max = Math.max(max, parseInt(numberPart));
        });

        instance[slugField] = slugBody + '-' + (max + 1);
...
module.exports.createUnusedSlug = createUnusedSlug;
