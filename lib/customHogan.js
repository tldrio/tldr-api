...
function readAndCompileTemplates (root, callback) {
  ...
  fs.readdir(templatesDir + root, function (err, dirContents) {
    if (err) { return callback(); }   // If we can't read the directory, it may mean it is in fact a file not ending with
                                      // the correct extension and thus mistaken for a directory in a lower recursion level. It may also mean
    ...
    // Once directory has been read, sort items between templates (= ending with the correct extension) and subdirectories
    _.each(dirContents, function (item) {
      if (item.indexOf(extension) ===  item.length - extension.length) {
    ...
    , function (cb) {
        fs.readFile(templatesDir + files[i], 'utf8', function (err, str) {
          ...
          compiledTemplates[stripExtension(files[i - 1])] = hogan.compile(str);
          cb();
        });
      }
    , function () {
...
// Remove the extension defined above from a given filename, if there is one.
function stripExtension (str) {
  if (str.indexOf(extension) === str.length - extension.length) {
    return str.substr(0, str.length - extension.length);
  } else {
    return str;
  }
}
...
function render (path, options, fn) {
  ...
  var templateToRender = compiledTemplates[stripTemplatesDir(stripExtension(path))];
