...
function populateArticleWordCount (tldr, cb) {
  ...
  request.get({ uri: uri }, function (error, response, body) {
    ...
    if (error) {
      bunyan.warn({ message: 'Error with the Readability API', error: error });
    ...
    if (!articleWordCount or isNaN(articleWordCount)) {
      bunyan.warn({ message: 'Response from the Readability API strange', error: error });
      return callback(error);
    }
