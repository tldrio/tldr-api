...
function populateArticleWordCount (tldr, cb) {
  ...
  if (config.env === 'test') { return; }   // We're nit here to test their API

  request.get({ uri: uri }, function (error, response, body) {
    var resJSON, articleWordCount;

    if (error) {
      console.log({ message: 'Error with the Readability API', error: error });
    ...
    if (!articleWordCount || isNaN(articleWordCount)) {
      console.log({ message: 'Response from the Readability API strange', error: error, body: body });
      return callback(error);
    }
