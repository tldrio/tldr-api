...
bunyanLogger.customLog = function (level, req, toLog) {
  ...
  if (! this.setToLog) { return; }   // If logger isn't explicitely set to log, do nothing (useful option to avoid huge logging clutter in test environment)
...
bunyanLogger.incrementMetric = function(counter, req) {
  ...
  if (! this.setToLog) { return; }

  message = new Buffer(counter + ":1|c");
