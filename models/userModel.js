...
UserSchema = new Schema(
  ...
  , lastWhitelistedTldr: { type: ObjectId, ref: 'tldr' }
  , lastWhitelistedTldrAt: { type: Date }
  , lastWalledHover: { type: Date, default: Date.now }
...
UserSchema.virtual('canHover').get(function () {
  var result = false;
  // Don't set this virtual if its not the complete profile that's requested
  if (!this.lastWalledHover) { return undefined; }
  result = result || (Date.now() - this.lastWalledHover.getTime() < 24 * 3600 * 1000);
  result = result || (this.lastWhitelistedTldrAt && (Date.now() - this.lastWhitelistedTldrAt.getTime() < 7 * 24 * 3600 * 1000));
  return result;
});
...
UserSchema.methods.getAuthorizedFields = function () {
  ...
  usableKeys.push('isAdmin');
  usableKeys.push('canHover');
...
/**
 * Allow a user to hover on HN for 24 hours
 * @param {Function} callback
 * @return {void}
 */
UserSchema.methods.allowHover = function (callback) {
  this.lastWalledHover = Date.now();
  this.save(callback);
};
